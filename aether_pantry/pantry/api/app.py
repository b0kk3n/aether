"""FastAPI application for Aether Pantry."""
...
from pantry.core import init_db, migrate_db
from pantry.api.routes import (
    categories_router,
    products_router,
    grocery_list_router,
    checklists_router,
...
# In the container, PANTRY_STATIC_DIR and PANTRY_TEMPLATE_DIR are set to
# explicit fixed paths by the Dockerfile. Locally, fall back relative to this file.
_THIS_DIR = Path(__file__).parent.parent  # pantry/
_STATIC_DIR = Path(os.environ.get("PANTRY_STATIC_DIR", _THIS_DIR / "web" / "static"))
_TEMPLATE_DIR = Path(os.environ.get("PANTRY_TEMPLATE_DIR", _THIS_DIR / "web" / "templates"))
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Aether Pantry",
        description="What's low or out at home, so it doesn't have to live in your head.",
        version=__version__,
        lifespan=lifespan,
    )
    ...
    async def serve_root(request: Request):
        ...
        if not index_path.exists():
            return {"message": "Aether Pantry API is running. Web UI not found."}
        ...
        html = html.replace(
            ...
            f'<script>\nwindow.AETHER_PANTRY_BASE = {json.dumps(ingress_path)};\n{js}\n</script>',
        )
