...
@router.post("/reorder", response_model=list[Category])
def reorder_categories(category_ids: list[str]):
    """Reorder categories by providing list of IDs in desired order."""
    return CategoryService.reorder(category_ids)


@router.delete("/{category_id}", status_code=204)
def delete_category(category_id: str):
    """Delete a category. Blocked if any chore still uses it."""
